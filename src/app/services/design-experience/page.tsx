...
export const metadata: Metadata = {
  title: 'Design & Experience - Morton Software Insights LLC',
  ...
  keywords: 'design experience, branding, logo design, UX UI design, design systems, user experience, visual identity',
};
...
export default function DesignExperiencePage() {
  ...
  return (
    <div className="min-h-screen bg-morton-purple-950">
          ...
          <AnimatedSection>
            <div className="text-center max-w-4xl mx-auto">
              <div className="inline-flex items-center space-x-2 bg-morton-orange/10 border border-morton-orange/30 rounded-full px-4 py-2 mb-8">
                <Palette className="h-4 w-4 text-morton-orange" />
                <span className="text-morton-orange font-medium">Design & Experience</span>
              </div>

              <h1 className="text-4xl lg:text-6xl font-bold text-white mb-6">
                Designs That 
                <span className="font-brand text-morton-orange block">Drive Results</span>
              </h1>
                ...
                {capabilities.map((capability, index) => (
                  <div key={capability.title} className="text-center">
                    <div className="p-3 bg-morton-orange/10 border border-morton-orange/30 rounded-xl w-fit mx-auto mb-3">
                      <capability.icon className="h-5 w-5 text-morton-orange" />
                    </div>
                    <h3 className="text-white font-semibold text-sm mb-1">{capability.title}</h3>
                    <p className="text-morton-purple-200 text-xs">{capability.description}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <AnimatedSection>
            <div className="text-center mb-16">
              <h2 className="text-3xl lg:text-4xl font-bold text-white mb-4">
                Design <span className="font-brand text-morton-orange">Services</span>
              </h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
            {services.map((service, index) => (
              <AnimatedSection key={service.name} delay={index * 0.1}>
                <div className="bg-morton-purple-950 border border-morton-purple-700 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 group hover:scale-[1.02] h-full relative">
                  {service.popular && (
      ...
      {/* Portfolio Section */}
      <section id="portfolio" className="py-20 bg-morton-purple-950">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <AnimatedSection>
            <div className="text-center mb-16">
              <h2 className="text-3xl lg:text-4xl font-bold text-white mb-4">
                Our Design <span className="font-brand text-morton-orange">Portfolio</span>
              </h2>
              <p className="text-lg text-morton-purple-200 max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {portfolio.map((project, index) => (
              <AnimatedSection key={project.title} delay={index * 0.1}>
                <div className="bg-morton-purple-900 border border-morton-purple-700 rounded-xl p-6 hover:scale-[1.02] transition-all duration-300 group">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="px-3 py-1 bg-morton-orange/10 border border-morton-orange/30 rounded-full">
                      <span className="text-morton-orange text-xs font-semibold">{project.metrics}</span>
                    </div>
                  </div>
                  <p className="text-morton-purple-200 leading-relaxed">{project.description}</p>
                </div>
              </AnimatedSection>
            ))}
          </div>
      ...
      <section className="py-20 bg-morton-purple-900">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <AnimatedSection>
            <div className="text-center mb-16">
              <h2 className="text-3xl lg:text-4xl font-bold text-white mb-4">
                Our Design <span className="font-brand text-morton-orange">Process</span>
              </h2>
              <p className="text-lg text-morton-purple-200 max-w-2xl mx-auto">
            ...
            <div className="space-y-8">
              {designProcess.map((step, index) => (
                <AnimatedSection key={step.step} delay={index * 0.1}>
                  <div className="flex items-center space-x-6 bg-morton-purple-950 border border-morton-purple-700 rounded-xl p-6 hover:scale-[1.02] transition-all duration-300 group">
                    <div className="text-4xl font-bold text-morton-orange/30 flex-shrink-0">
                      {step.step}
                    </div>
                    <div className="flex-grow">
                      <h3 className="text-xl font-bold text-white mb-2 group-hover:text-morton-orange transition-colors">
                        {step.title}
                      </h3>
                    ...
                    <div className="p-3 bg-morton-orange/10 border border-morton-orange/30 rounded-xl flex-shrink-0">
                      <step.icon className="h-6 w-6 text-morton-orange" />
                    </div>
                  </div>
                </AnimatedSection>
              ))}
      ...
      <section className="py-20 bg-morton-purple-950">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <AnimatedSection>
            <div className="text-center mb-16">
              <h2 className="text-3xl lg:text-4xl font-bold text-white mb-4">
                Client <span className="font-brand text-morton-orange">Love</span>
              </h2>
              <p className="text-lg text-morton-purple-200 max-w-2xl mx-auto">
            ...
            <div className="bg-morton-purple-950 border border-morton-purple-700 rounded-2xl p-8 lg:p-12 text-center">
              <div className="flex items-center justify-center space-x-3 mb-6">
                <div className="p-3 bg-morton-orange/10 border border-morton-orange/30 rounded-xl">
                  <Palette className="h-6 w-6 text-morton-orange" />
                </div>
                <h2 className="text-3xl lg:text-4xl font-bold text-white">
                  Ready to Create Amazing <span className="font-brand text-morton-orange">Designs</span>?
                </h2>
              </div>
